...
from utils.general import check_img_size, non_max_suppression, apply_classifier, scale_coords, xyxy2xywh, \
    strip_optimizer, set_logging, increment_path, indent, check_cls
...
def detect(save_img=False):
    source, weights, view_img, save_txt, save_xml, imgsz = opt.source, opt.weights, opt.view_img, opt.save_txt, opt.save_xml, opt.img_size
    ...
    (save_dir / 'xmls' if save_xml else save_dir).mkdir(parents=True, exist_ok=True)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_xml:  # Write file as xml format
