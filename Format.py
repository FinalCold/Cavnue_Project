...
class VOC:
    ...
    def parse(path, volume):
        try:
            filelist = glob.glob(path + '**/*.xml', recursive=True)
            filenum = len(filelist)
            
            filenames = filelist[::volume]
            
            # filenames = random.sample(filelist, volume)
...
class YOLO:
    ...
    def save(self, data, save_path, img_path, img_type, manifest_path):
        ...
        try:
            ...
            imagelist = glob.glob(img_path + '**/*.jpg', recursive=True)
            flag = 0                
            for key in data:
                angle = key[0]
                folder = key[2:17]
                ...
                abs_train_data_path = os.path.join(abs_save_path, 'train/')
                abs_val_data_path = os.path.join(abs_save_path, 'val/')

                abs_train_image_save_path = os.path.join(abs_train_data_path, 'images/')
                abs_train_label_save_path = os.path.join(abs_train_data_path, 'labels/')
                
                abs_val_image_save_path = os.path.join(abs_val_data_path, 'images/')
                abs_val_label_save_path = os.path.join(abs_val_data_path, 'labels/')
                
                try:
                
                    Path(abs_train_image_save_path).mkdir(parents=True, exist_ok=True)
                    Path(abs_train_label_save_path).mkdir(parents=True, exist_ok=True)
                    Path(abs_val_image_save_path).mkdir(parents=True, exist_ok=True)
                    Path(abs_val_label_save_path).mkdir(parents=True, exist_ok=True)
                    

                    if key in str(imagelist):

                        
                        match = [s for s in imagelist if key in s]
                        
                        if flag < 10:
                          
                            shutil.copy(match[0], abs_train_image_save_path)
                            
                            with open(os.path.join(abs_train_label_save_path, "".join([key, ".txt"])), "w") as output_txt_file:
                                output_txt_file.write(data[key])
                            flag += 1
                                
                        elif flag == 10:
                            shutil.copy(match[0], abs_val_image_save_path)
                            
                            with open(os.path.join(abs_val_label_save_path, "".join([key, ".txt"])), "w") as output_txt_file:
                                output_txt_file.write(data[key])
                            flag = 0
                    else:
                        print(key, "No matching IMAGE !!!") 
